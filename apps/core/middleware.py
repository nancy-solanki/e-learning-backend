...
class UserStatusMiddleware:
        ...
        if token:
            try:
                ...
                if user.status == User.Status.SUSPEND:
                    return self._build_error_response(
                        _("Account is not active"), status.HTTP_403_FORBIDDEN
                    )
                ...
                if user.password_changed_at:
                    ...
                    if iat < password_changed_timestamp:
                        return self._build_error_response(
                            _("Credentials are Invalid"), status.HTTP_401_UNAUTHORIZED
                        )
    ...
    def _build_error_response(self, message, status_code):
        response = Response(
            {"errors": {"non_field_error": [message]}}, status=status_code
        )
