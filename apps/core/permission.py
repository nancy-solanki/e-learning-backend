...
class IsInstructorOrAdmin(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        try:
            ...
            return request.user.groups.filter(name="instructor").exists() or request.user.groups.filter(name="admin").exists()
        except:
            return False

    def has_object_permission(self, request, view, obj):
        return request.user == obj.instructor or request.user.groups.filter(name="admin").exists()
...
class IsInstructorOrReadOnly(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        return request.user == instructor or is_instructor_user or request.user.groups.filter(name="admin").exists()
...
class IsOwnerOrInstructor(permissions.BasePermission):
    """
    Permission to allow either the resource owner (student) or the course/resource instructor to modify.
    """
    def has_permission(self, request, view):
        return request.user and request.user.is_authenticated
    ...
    def has_object_permission(self, request, view, obj):
        if request.method in permissions.SAFE_METHODS:
            return True
        return (
            IsOwnerOrReadOnly().has_object_permission(request, view, obj) or
            IsInstructorOrReadOnly().has_object_permission(request, view, obj)
        )
